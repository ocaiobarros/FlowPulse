...
interface CTOTelemetry {
  id: string; status: string; healthRatio: number;
  onuOnline: number; onuOffline: number; onuAuthorized: number; onuUnprovisioned: number;
  ponLinkStatus: string; trafficIn: number | null; trafficOut: number | null;
  temperature: number | null; fanStatus: string | null; fanRotation: number | null;
  txPower: number | null; cpuLoad: number | null; uptime: number | null;
  isMassiva: boolean;
}

interface SlotTemp { slot: string; temperature: number }
interface TopPon { pon: string; trafficBps: number; hostId: string }
...
interface OLTHealth {
  ...
  slotTemperatures: SlotTemp[];
  topPons: TopPon[];
}

/* ─── Item classification helpers ─── */
function classifyItems(items: Array<{ key_: string; lastvalue: string; name: string; hostid: string }>) {
  const byHost = new Map<string, Array<{ key_: string; lastvalue: string; name: string }>>();
  for (const item of items) {
    if (!byHost.has(item.hostid)) byHost.set(item.hostid, []);
    byHost.get(item.hostid)!.push({ key_: item.key_, lastvalue: item.lastvalue, name: item.name });
  }
  return byHost;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!ctos || ctos.length === 0) return json({ updated: 0, ctos: [], oltHealth: {} });
    ...
    const oltZbxMap = new Map<string, string>();
    ...
    if (allZbxIds.size === 0) return json({ updated: 0, ctos: [], oltHealth: {} });
    ...
    const [zbxHosts, zbxItems] = await Promise.all([
      ...
      }) as Promise<Array<{ hostid: string; host?: string; name?: string; available?: string; interfaces?: Array<{ available: string }> }>>,

      zabbixCall(conn.url, zabbixAuth, "item.get", {
        hostids: zbxHostIds,
        output: ["itemid", "hostid", "key_", "lastvalue", "name"],
        search: { key_: "pon,ramal,descoberta,fan,CPU,net.if,desprovisionadas,1.3.6.1.2.1.1.3,1.3.6.1.4.1.2011.6.128.1.1.2.23.1.4,1.3.6.1.4.1.2011.6.1.1.5.1.9,1.3.6.1.4.1.2011.6.3.3.2.1.13,1.3.6.1.4.1.2011.6.1.1.5.1.6,1.3.6.1.4.1.2011.2.6.7.1.1.2.1.5" },
        searchByAny: true,
        limit: 5000,
    ...
    for (const h of zbxHosts) {
      hostNameMap.set(h.hostid, h.name || h.host || h.hostid);
      let up = false;
    ...
    const itemsByHost = classifyItems(zbxItems.map(i => ({ ...i })));
    ...
    for (const cto of ctos) {
      ...
      let onuOnline = 0, onuOffline = 0, onuAuthorized = 0, onuUnprovisioned = 0;
      let ponLinkStatus = "UNKNOWN";
      let trafficIn: number | null = null, trafficOut: number | null = null;
      let temperature: number | null = null, fanStatus: string | null = null, fanRotation: number | null = null;
      let txPower: number | null = null, cpuLoad: number | null = null, uptime: number | null = null;
      ...
      for (const zbxId of relevantIds) {
        ...
        for (const item of items) {
          ...
          if (key.includes("pon[") && key.includes(",\"1\",")) { onuOnline += parseInt(val) || 0; }
          else if (key.includes("pon[") && key.includes(",\"2\",")) { onuOffline += parseInt(val) || 0; }
          else if (key.includes("ramal.autorizadas")) { onuAuthorized += parseInt(val) || 0; }
          else if (key.includes("desprovisionadas")) { onuUnprovisioned += parseInt(val) || 0; }
            ...
            if (!isNaN(pw)) txPower = txPower == null ? pw : Math.min(txPower, pw);
          }
          else if (key.includes("net.if.in[ifhcinoctets")) { trafficIn = (trafficIn ?? 0) + (parseFloat(val) || 0); }
          else if (key.includes("net.if.out[ifhcoutoctets")) { trafficOut = (trafficOut ?? 0) + (parseFloat(val) || 0); }
          else if (key.includes("1.3.6.1.4.1.2011.6.3.3.2.1.13")) {
            const temp = parseInt(val) || 0;
            if (temp > 0 && temp < 2147483647) temperature = temperature == null ? temp : Math.max(temperature, temp);
          }
          else if (key.includes("1.3.6.1.4.1.2011.6.1.1.5.1.6")) { fanStatus = val === "1" ? "ACTIVE" : "INACTIVE"; }
          ...
          else if (key.includes("cpu[")) { const cpu = parseFloat(val) || 0; cpuLoad = cpuLoad == null ? cpu : Math.max(cpuLoad, cpu); }
          else if (key.includes("1.3.6.1.2.1.1.3.0")) { const ut = parseFloat(val) || 0; uptime = uptime == null ? ut : Math.max(uptime, ut); }
        }
      }
      ...
      if (ponLinkStatus === "DOWN") newStatus = "CRITICAL";
      else if (healthRatio >= 0.9) newStatus = "OK";
      else if (healthRatio >= 0.1) newStatus = "DEGRADED";
      else newStatus = "CRITICAL";

      if (totalAuthorized === 0 && directIds.length > 0) {
        let upCount = 0;
        for (const zbxId of directIds) { if (availMap.get(zbxId)) upCount++; }
        const ratio = upCount / directIds.length;
        newStatus = ratio >= 0.9 ? "OK" : ratio >= 0.1 ? "DEGRADED" : "CRITICAL";
      }

      // Massiva: health drops >50%
      const isMassiva = totalAuthorized > 0 && healthRatio < 0.5;
      ...
      ctoTelemetry.push({
        id: cto.id, status: newStatus,
        healthRatio: Math.round(healthRatio * 1000) / 10,
        onuOnline, onuOffline, onuAuthorized: totalAuthorized, onuUnprovisioned,
        ponLinkStatus, trafficIn, trafficOut, temperature, fanStatus, fanRotation,
        txPower, cpuLoad, uptime, isMassiva,
      });
    }
    ...
    if (updates.length > 0) {
      await Promise.all(updates.map((upd) =>
        serviceClient.from("flow_map_ctos").update({ status_calculated: upd.newStatus }).eq("id", upd.id)
      ));
    }

    // Aggregate OLT-level health with slot temperatures + Top 5 PONs
    const oltHealthMap = new Map<string, OLTHealth>();
    for (const zbxId of allZbxIds) {
      const items = itemsByHost.get(zbxId) ?? [];
      const health: OLTHealth = {
        hostId: zbxId, hostName: hostNameMap.get(zbxId) ?? zbxId,
        temperature: null, fanStatus: null, fanRotation: null, cpuLoad: null, uptime: null,
        totalOnuOnline: 0, totalOnuOffline: 0, totalUnprovisioned: 0,
        slotTemperatures: [], topPons: [],
      };
      ...
      for (const item of items) {
        ...
        // Slot temperatures — collect per-slot
        if (key.includes("1.3.6.1.4.1.2011.6.3.3.2.1.13")) {
          const t = parseInt(val) || 0;
          if (t > 0 && t < 2147483647) {
            health.temperature = health.temperature == null ? t : Math.max(health.temperature, t);
            // Extract slot index from key
            const slotMatch = key.match(/\.(\d+)$/);
            const slotLabel = slotMatch ? `Slot ${slotMatch[1]}` : item.name || "Slot";
            health.slotTemperatures.push({ slot: slotLabel, temperature: t });
          }
        }
        else if (key.includes("1.3.6.1.4.1.2011.6.1.1.5.1.6")) { health.fanStatus = val === "1" ? "ACTIVE" : "INACTIVE"; }
        else if (key.includes("1.3.6.1.4.1.2011.6.1.1.5.1.9")) {
        ...
        else if (key.includes("1.3.6.1.2.1.1.3.0")) { health.uptime = parseFloat(val) || 0; }
        else if (key.includes("pon[") && key.includes(",\"1\",")) { health.totalOnuOnline += parseInt(val) || 0; }
        else if (key.includes("pon[") && key.includes(",\"2\",")) { health.totalOnuOffline += parseInt(val) || 0; }
        else if (key.includes("desprovisionadas")) { health.totalUnprovisioned += parseInt(val) || 0; }
        // Top PONs: collect traffic per PON interface
        else if (key.includes("net.if.in[ifhcinoctets")) {
          const bps = parseFloat(val) || 0;
          if (bps > 0) {
            const ponLabel = item.name || key;
            health.topPons.push({ pon: ponLabel, trafficBps: bps, hostId: zbxId });
          }
        }
      }

      // Sort top PONs by traffic descending, keep top 5
      health.topPons.sort((a, b) => b.trafficBps - a.trafficBps);
      health.topPons = health.topPons.slice(0, 5);

      // Sort slot temps by slot label
      health.slotTemperatures.sort((a, b) => a.slot.localeCompare(b.slot, undefined, { numeric: true }));
