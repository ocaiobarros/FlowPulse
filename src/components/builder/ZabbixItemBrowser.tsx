...
import { Search, Server, FolderTree, Activity, ChevronDown, Check, ChevronRight, Plus, Loader2, RefreshCw } from "lucide-react";
...
export default function ZabbixItemBrowser({ connectionId, selectedItemId, initialGroupId, initialHostId, initialGroupName, initialHostName, initialItemName, multiSelect, selectedSeries, onSelectItem }: ZabbixItemBrowserProps) {
  ...
  const [isLargeHost, setIsLargeHost] = useState(false);
  ...
  // Debounce: only triggers server search for large hosts
  useEffect(() => {
    if (!isLargeHost) return;
    clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => {
      setServerSearch(itemSearch);
    }, 500);
    return () => clearTimeout(debounceRef.current);
  }, [itemSearch, isLargeHost]);
  ...
  useEffect(() => {
    ...
    getNavCache(groupsCacheKey).then((cached) => {
      if (cached && cached.length > 0) queryClient.setQueryData(["zabbix-groups", connectionId], cached);
    }).catch(() => {});
  }, [connectionId]);
  ...
  useEffect(() => {
    ...
    getNavCache(hostsCacheKey).then((cached) => {
      if (cached && cached.length > 0) queryClient.setQueryData(["zabbix-hosts", connectionId, selectedGroup], cached);
    }).catch(() => {});
  }, [connectionId, selectedGroup]);

  // ── Items: initial load (no filter, limit 100) ──
  const { data: initialItems = [], isLoading: loadingItems, isFetching: fetchingInitial } = useQuery({
    queryKey: ["zabbix-items-init", connectionId, selectedHost],
    queryFn: async () => {
      ...
      // If we got exactly INITIAL_LIMIT items, the host likely has more
      setIsLargeHost(items.length >= INITIAL_LIMIT);
      return items;
    },
  ...
  // ── Server-side search (only for large hosts when user types) ──
  const { data: serverItems, isFetching: fetchingServer } = useQuery({
    queryKey: ["zabbix-items-search", connectionId, selectedHost, serverSearch],
    queryFn: async () => {
      const result = await zabbixProxy(connectionId!, "item.get", {
        output: ["itemid", "key_", "name", "lastvalue", "units", "value_type"],
        hostids: [selectedHost],
        sortfield: "name",
        limit: INITIAL_LIMIT,
        search: { name: serverSearch.trim() },
        startSearch: true,
      });
      return (result as ZabbixItem[]) || [];
    },
    enabled: !!connectionId && !!selectedHost && isLargeHost && !!serverSearch.trim(),
    staleTime: 60 * 1000,
  });

  // ── Hybrid filtering ──
  const displayItems = useMemo(() => {
    // Large host + user typed + server results available
    if (isLargeHost && serverSearch.trim() && serverItems) {
      return serverItems;
    }
    // Small host or no search: filter locally
    if (!itemSearch.trim()) return initialItems;
    const fuse = new Fuse(initialItems, FUSE_OPTIONS);
    return fuse.search(itemSearch).map((r) => r.item);
  }, [initialItems, serverItems, itemSearch, serverSearch, isLargeHost]);

  // ── Load all items (fallback button) ──
  const [loadingAll, setLoadingAll] = useState(false);
  const handleLoadAll = useCallback(async () => {
    if (!connectionId || !selectedHost) return;
    setLoadingAll(true);
    try {
      const result = await zabbixProxyPaginated(connectionId, "item.get", {
        output: ["itemid", "key_", "name", "lastvalue", "units", "value_type"],
        hostids: [selectedHost],
        sortfield: "name",
      });
      queryClient.setQueryData(["zabbix-items-init", connectionId, selectedHost], result);
      setIsLargeHost(false); // now all items are local
    } catch (err) {
      console.error("Load all items failed:", err);
    } finally {
      setLoadingAll(false);
    }
  }, [connectionId, selectedHost, queryClient]);
  ...
  const handleSelect = useCallback((item: ZabbixItem) => {
    onSelectItem(item, { groupId: selectedGroup, hostId: selectedHost, groupName, hostName });
  ...
  const isBusy = fetchingInitial || fetchingServer || loadingAll;
  ...
  return (
    <div className="space-y-2">
      ...
      {selectedHost && (
        <div className="space-y-1">
          <Label className="text-[10px] text-muted-foreground flex items-center gap-1">
            <Activity className="w-3 h-3" /> Itens ({displayItems.length})
            {isBusy && <Loader2 className="w-3 h-3 animate-spin text-primary ml-1" />}
            {isLargeHost && !isBusy && (
              <span className="text-[8px] text-muted-foreground/60 ml-1">(host grande — busca via API)</span>
            )}
          </Label>
          <div className="relative">
            <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground" />
            <Input
              ...
              placeholder={isLargeHost ? "Buscar no servidor…" : "Busca fuzzy…"}
          ...
          {loadingItems ? (
            ...
            <div ref={parentRef} className="h-[320px] border border-border/30 rounded-md overflow-auto">
              ...
              {displayItems.length === 0 && !loadingItems && !isBusy && (
                <div className="flex flex-col items-center justify-center py-4 gap-2">
                  <p className="text-[9px] text-muted-foreground">Nenhum item encontrado</p>
                  <button
                    type="button"
                    onClick={handleLoadAll}
                    disabled={loadingAll}
                    className="text-[9px] text-primary hover:text-primary/80 font-medium flex items-center gap-1 border border-primary/30 rounded px-2 py-1"
                  >
                    {loadingAll ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />}
          ...
          {isLargeHost && displayItems.length > 0 && (
            <button
              type="button"
              onClick={handleLoadAll}
              disabled={loadingAll}
              className="text-[8px] text-muted-foreground hover:text-primary flex items-center gap-1 px-1"
            >
              {loadingAll ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <RefreshCw className="w-2.5 h-2.5" />}
              Carregar todos os itens do host
            </button>
          )}
        </div>
      )}
