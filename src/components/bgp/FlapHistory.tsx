...
/* ─── Generate mock flap data from peers ── */
  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground/40">
        ...
        <p className="text-xs font-mono">Sem dados de estabilidade</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-mono font-semibold text-foreground flex items-center gap-2">
          ...
          Histórico de Flaps — Últimas 24h
        </h3>
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-mono text-muted-foreground/60">
            Total: <span className={`font-bold ${totalFlaps > 10 ? "text-[hsl(var(--neon-red))]" : totalFlaps > 3 ? "text-[hsl(var(--neon-amber))]" : "text-[hsl(var(--neon-green))]"}`}>
              {totalFlaps} flaps/dia
            </span>
          </span>
          <div className="flex items-center gap-2 text-[9px] font-mono text-muted-foreground/40">
            <span className="flex items-center gap-1">
              <div className="w-2 h-2 rounded-full bg-[hsl(var(--neon-red))]" /> Reset
            </span>
            <span className="flex items-center gap-1">
              <div className="w-2 h-2 rounded-full bg-[hsl(var(--neon-amber))]" /> Flap
            </span>
          </div>
        </div>
      </div>
          ...
          {data.map((peer, i) => {
            ...
            return (
              <motion.div
                ...
                className={`flex items-center border-b border-border/10 hover:bg-muted/5 transition-colors ${
                  hasHighFlaps ? "bg-[hsl(var(--neon-red)/0.03)]" : ""
                }`}
              >
                {/* Peer label */}
                <div className="w-32 px-3 py-2.5 shrink-0 flex items-center gap-2">
                  <span className="text-[10px] font-mono text-foreground truncate font-medium">
                    {peer.name}
                  </span>
                  {hasHighFlaps && (
                    <AlertTriangle className="w-3 h-3 text-[hsl(var(--neon-red))] shrink-0" />
                  )}
                </div>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          boxShadow: isReset
                            ? "0 0 6px hsl(var(--neon-red) / 0.5)"
                            : "0 0 4px hsl(var(--neon-amber) / 0.4)",
                        }}
                        title={`${isReset ? "Reset" : "Flap"} — ${new Date(flap.time).toLocaleTimeString("pt-BR")}`}
                      />
                    );
                  })}
                </div>
