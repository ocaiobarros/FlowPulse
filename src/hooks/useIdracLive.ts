...
export interface IdracData {
  ...
  /** Detected model prefix (e.g. "Dell R720: ") or "" */
  prefix: string;
}
...
function detectPrefix(items: Map<string, ZabbixItem>): string {
  // Look for "Dell XXXX: " prefix pattern
...
/**
 * Search items map with multiple candidate names.
 * Supports both T440-style flat names and R720-style "Dell R720: " prefixed names.
 */
function findValue(d: IdracData, ...candidates: string[]): string {
  ...
  // Try any prefix "Dell XXXX: " pattern
  for (const c of candidates) {
...
export function extractFans(d: IdracData) {
  ...
  for (let i = 1; i <= 10; i++) {
    // T440: "Fan System Board Fan1 Speed" | R720: "Dell R720: System Board Fan1 Speed"
...
export function extractPower(d: IdracData) {
  ...
  for (let i = 1; i <= 4; i++) {
    // T440: "Power Supply 1 Status" | R720: "Dell R720: PS1 Status"
...
export function extractDisks(d: IdracData) {
  ...
  // Pattern 2: R720 — "Physical Disk 0:1:N ..." and "Solid State Disk 0:1:N ..."
  if (disks.length === 0) {
    const prefix = d.prefix;
    const diskPatterns = [
      /^(?:.*:\s)?(?:Physical Disk|Solid State Disk)\s+([\d:]+)\s+(.+)$/,
    ];

    // Collect unique disk identifiers
    ...
    for (const [diskId, fields] of diskMap) {
      disks.push({
        ...
        smartStatus: fields.smartStatus || "",
      });
    }
  }
...
export function extractRaid(d: IdracData) {
  ...
  // Pattern 2: R720 — virtual disks by name (e.g. "Dados", "SSD1", "TrueNAS")
  if (volumes.length === 0) {
    const prefix = d.prefix;
    ...
    for (const vdName of vdNames) {
      ...
      volumes.push({
        ...
        name: vdName,
        size: size,
        state: findValue(d, `${vdName} Current state`),
        status: status,
        vdState: status,
...
export function extractMemory(d: IdracData) {
  ...
  // T440: "Memory Slot DIMM.Socket.A1 Size"
  // R720: might have different patterns
  const memPattern = /^(?:.*:\s)?Memory Slot\s+(.+?)\s+(Size|Speed|Manufacturer|Part Number|Serial Number|Status)$/;
